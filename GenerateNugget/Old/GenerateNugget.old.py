...
for el in els:
    ...
    if(ac.get_user_attribute(el,cube_attr)=='nugget node'):    
        ...
        for v in verts:
            ...
            p3 = cadwork.point_3d(x+half_x,y,z+half_z) # z offset, keep x offset and y origin
            ...
            p3b = p3+cadwork.point_3d(cutit_adds,0,cutit_adds*2+boundary_size_z)
            ...
            if(len(copy_set)):
                ...
                if(do_deletes):
                        ...
                        p1b = p1+cadwork.point_3d(-1*cutit_adds-5,-5,-5)
                        p2b = p2+cadwork.point_3d(cutit_adds+5,-5,-5)
                        p3b = p3+cadwork.point_3d(cutit_adds+5,-5,cutit_adds*2+boundary_size_z+5+(copy_z_offset + boundary_size_z))
                        cleanup_id = ec.create_square_beam_points(2*cutit_adds+boundary_size_y+10,p1b,p2b,p3b)
                        ac.set_user_attribute([cutit_id],cube_attr,'cleanup nugget')
                        ac.set_name([cutit_id],'nugget cleanup')
                        if(debug_mode):
                        ...
                        for e in copy_set2: 
                            verts = gc.get_element_vertices(e)
                            for v in verts:      
                                    if(ec.check_if_point_is_in_element(v,cleanup_id)):
                                            if(not ec.check_if_point_is_in_element(v,boundary_copy)):
